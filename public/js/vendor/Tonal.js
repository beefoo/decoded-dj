...
var height = ({ step, alt, oct, dir = 1 }) =>
  dir * (SIZES[step] + alt + 12 * (oct === void 0 ? -100 : oct));
var midi = (pitch2) => {
...
function isPitch(pitch2) {
  return pitch2 !== null &&
    typeof pitch2 === 'object' &&
    'step' in pitch2 &&
    typeof pitch2.step === 'number' &&
    'alt' in pitch2 &&
    typeof pitch2.alt === 'number' &&
    !isNaN(pitch2.step) &&
    !isNaN(pitch2.alt)
    ? true
    : false;
...
function scientificToAbcNotation(str) {
  ...
  const o =
    oct === 5 ? '' : oct > 4 ? fillStr3("'", oct - 5) : fillStr3(',', 4 - oct);
  return a + l + o;
}
function transpose2(note2, interval2) {
  return scientificToAbcNotation(
    transpose(abcToScientificNotation(note2), interval2),
  );
}
...
function chromaToPcset(chroma22) {
  ...
  const normalizedNum = chromaRotations(chroma22)
    .map(chromaToNumber)
    .filter((n) => n >= 2048)
    .sort()[0];
...
data_default.forEach(([ivls, fullName, names22]) =>
  add(ivls.split(' '), names22.split(' '), fullName),
);
...
function detect(source, options = {}) {
  ...
  return found
    .filter((chord2) => chord2.weight)
    .sort((a, b) => b.weight - a.weight)
    .map((chord2) => chord2.name);
}
var BITMASK = {
  // 3m 000100000000
  // 3M 000010000000
...
var testChromaNumber = (bitmask) => (chromaNumber) =>
...
function findMatches(notes4, weight, options) {
  ...
  allModes.forEach((mode2, index4) => {
    const modeWithPerfectFifth =
      options.assumePerfectFifth && withPerfectFifth(mode2);
    const chordTypes = all().filter((chordType2) => {
      if (
        options.assumePerfectFifth &&
        hasAnyThirdAndPerfectFifthAndAnySeventh(chordType2)
      ) {
...
data_default2.forEach(([ivls, name3, ...aliases]) =>
  add3(ivls.split(' '), name3, aliases),
);
...
function getChord(typeName, optionalTonic, optionalBass) {
  ...
  const notes22 = tonic.empty
    ? []
    : intervals2.map((i) => transpose(tonic.pc, i));
...
function chordScales(name3) {
  ...
  return all2()
    .filter((scale2) => isChordIncluded(scale2.chroma))
...
function extended(chordName) {
  ...
  return all()
    .filter((chord2) => isSuperset(chord2.chroma))
...
function reduced(chordName) {
  ...
  return all()
    .filter((chord2) => isSubset(chord2.chroma))
    .map((chord2) => s.tonic + chord2.aliases[0]);
}
...
function degrees(chordName, tonic) {
  ...
  return (degree) =>
    degree ? transpose22(degree > 0 ? degree - 1 : degree) : '';
}
...
var VALUES = [];
data_default3.forEach(([denominator, shorthand, names22]) =>
  add4(denominator, shorthand, names22),
);
var NoDuration = {
  empty: true,
  name: '',
...
function pcset2(notes4) {
  return Array.isArray(notes4)
    ? pcsetFromMidi(notes4)
    : pcsetFromChroma(notes4);
}
...
function get8(src) {
  return typeof src === 'string'
    ? cache4[src] || (cache4[src] = parse3(src))
    : typeof src === 'number'
      ? get8(NAMES2[src] || '')
      : isPitch(src)
        ? fromPitch(src)
...
var mapScaleToType = (scale2, list, sep = '') =>
  list.map((type, i) => `${scale2[i]}${sep}${type}`);
function keyScale(
  grades,
  triads3,
  chordTypes,
  ...
  return (tonic) => {
    ...
    const secondaryDominants = scale2
      .map((note2) => transpose4(note2, '5P'))
      .map((note2) =>
        // A secondary dominant is a V chord which:
        // 1. is not diatonic to the key,
        // 2. it must have a diatonic root.
        scale2.includes(note2) && !chords2.includes(note2 + '7')
          ? note2 + '7'
          : '',
      );
...
function keyChordsOf(key, chords2) {
  ...
  key.chords.forEach((chordName, index4) =>
    updateChord(chordName, key.chordsHarmonicFunction[index4]),
  );
  key.secondaryDominants.forEach((chordName, index4) =>
    updateChord(chordName, `V/${key.grades[index4]}`),
  );
  key.secondaryDominantSupertonics.forEach((chordName, index4) =>
    updateChord(chordName, `ii/${key.grades[index4]}`),
  );
  key.substituteDominants.forEach((chordName, index4) =>
    updateChord(chordName, `subV/${key.grades[index4]}`),
  );
  key.substituteDominantSupertonics.forEach((chordName, index4) =>
    updateChord(chordName, `subii/${key.grades[index4]}`),
  );
}
...
function get9(name3) {
  return typeof name3 === 'string'
    ? index3[name3.toLowerCase()] || NoMode
    : name3 && name3.name
      ? get9(name3.name)
      : NoMode;
}
...
function getNoteNameOf(scale2) {
  const names22 = Array.isArray(scale2)
    ? scaleNotes(scale2)
    : get10(scale2).notes;
...
var topNoteDiff = (voicings, lastVoicing) => {
  ...
  const topNoteMidi = (voicing) =>
    index_default2.midi(voicing[voicing.length - 1]) || 0;
  const diff = (voicing) =>
...
function lookup(symbol, dictionary3 = defaultDictionary) {
  ...
  const match =
    Object.keys(dictionary3).find((_symbol) => aliases.includes(_symbol)) || '';
  if (match !== void 0) {
...
function get12(
  chord2,
  range3 = defaultRange,
  dictionary3 = defaultDictionary2,
...
function search(
  chord2,
  range3 = defaultRange,
  dictionary3 = index_default7.triads,
) {
  ...
  return voicings.reduce((voiced, voicing) => {
    ...
    const starts = notesInRange
      .filter(
        (note2) =>
          index_default2.chroma(note2) ===
          index_default2.chroma(bottomPitchClass),
      )
      .filter(
        (note2) =>
          (index_default2.midi(
    ...
    const notes4 = starts.map((start) =>
      relativeIntervals.map((interval2) =>
        index_default2.transpose(start, interval2),
      ),
    );
